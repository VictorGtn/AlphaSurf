...
- `masif_ligand/` — MaSIF-Ligand figures: performance vs throughput (alone and
  ...
  `plot_perf_vs_throughput.py` is the standalone MaSIF-Ligand figure and needs
  nothing else. `plot_perf_vs_throughput_combined.py` overlays it on PINDER and
  reads `figures/pinder_pair/perf_vs_throughput_repaired_common_all_summary.csv`
  and `..._repaired_common_homo_hetero_summary.csv` by name. Those names are not
  the default, so produce them first:

  ```bash
  python pinder_pair/plot_perf_vs_throughput_seeds.py \
    --output figures/pinder_pair/perf_vs_throughput_repaired_common_all
  python pinder_pair/plot_perf_vs_throughput_seeds.py --homo-hetero \
    --output figures/pinder_pair/perf_vs_throughput_repaired_common_homo_hetero
  ```
