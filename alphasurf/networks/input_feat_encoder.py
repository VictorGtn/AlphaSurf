...
from alphasurf.network_utils.communication.utils_blocks import init_block
        ...
        if self.use_neigh:
            ...
            with torch.no_grad():
                ...
                for x in all_dists:
                    ...
                    neighbors.append(
                        torch.stack((repeated_tensor, min_indices.flatten()))
                    )
            ...
            expanded_message_features = torch.cat(
                [all_chem_feats, encoded_dists, encoded_angles], axis=-1
            )
...
class InputEncoder(SurfaceGraphCommunication):
    def __init__(
        ...
        **kwargs,
        ...
        gvp_use_angles = (
            hparams.gvp_use_angles if "gvp_use_angles" in hparams else False
        )
        ...
        if use_bp:
            if use_gvp:
                ...
                bp_sg_block = init_block(
                    "gcn",
                    use_gat=hparams.use_gat,
                    use_v2=hparams.use_v2,
                    dim_in=hparams.bp_s_dim_in,
                    dim_out=hparams.bp_s_dim_out,
                    add_self_loops=hparams.bp_self_loops,
                    fill_value=hparams.bp_fill_value,
                )
                bp_gs_block = init_block(
                    "gcn",
                    use_gat=hparams.use_gat,
                    use_v2=hparams.use_v2,
                    dim_in=hparams.bp_g_dim_in,
                    dim_out=hparams.bp_g_dim_out,
                    add_self_loops=hparams.bp_self_loops,
                    fill_value=hparams.bp_fill_value,
                )
        else:
        ...
        super().__init__(
            ...
            neigh_thresh=hparams.neigh_thresh,
            sigma=hparams.sigma,
            use_knn=hparams.use_knn,
            **kwargs,
        )
    ...
    def forward(self, surface, graph):
        ...
        with torch.no_grad():
            ...
            for x in all_dists:
                ...
                offset_surf += len(x)
                offset_graph += len(x.T)
                neighbors_surf.append(
                    torch.stack((repeated_tensor_vert, min_indices_vert.flatten()))
                )
                neighbors_graph.append(
                    torch.stack((repeated_tensor_graph, min_indices_graph.flatten()))
                )
        ...
        encoded_angles = _rbf(nbr_angular, D_min=-1, D_max=1, D_count=self.num_gdf)
        expanded_message_features = torch.cat(
            [all_geom_feats, encoded_dists, encoded_angles], axis=-1
        )
