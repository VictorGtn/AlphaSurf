...
def curvatures(
    ...
    """
    use_keops = True  # Set to True to restore the original KeOps path.
    n_points = vertices.shape[0]

    if use_keops:
        with time_operation("curvatures_keops", {"points": n_points}):
            # Number of points, number of scales:
            N, S = n_points, len(scales)
    ...
    # Torch-only path without KeOps.
    with time_operation("curvatures_torch", {"points": n_points}):
        scale_list = list(scales) if hasattr(scales, "__len__") else [scales]
        N = n_points
        ...
        unique_batches = batch_vec.unique(sorted=True)

        # Base normals and areas.
        if triangles is not None:
            A = vertices[triangles[0, :]]
            B = vertices[triangles[1, :]]
            C = vertices[triangles[2, :]]
            face_normals = torch.cross(B - A, C - A)
            face_areas = face_normals.norm(dim=-1) / 2.0
            safe_areas = face_areas.unsqueeze(-1).clamp_min(1e-12)
            face_normals_unit = face_normals / safe_areas
            ...
            for idx in range(3):
                vert_normals.index_add_(
                    0,
                    triangles[idx, :],
                    face_normals_unit * (face_areas[:, None] / 3.0),
                )
            vert_normals = F.normalize(vert_normals, p=2, dim=-1)
        else:
            face_areas = None
            if normals is None:
                raise ValueError("Normals are required when triangles are not provided")
            vert_normals = F.normalize(normals, p=2, dim=-1)

        normals_s = torch.zeros((N, len(scale_list), 3), device=device, dtype=dtype)
        for b in unique_batches:
            mask = batch_vec == b
            if mask.sum() == 0:
                continue
            v_batch = vertices[mask]
            n_batch = vert_normals[mask]
            dist2 = torch.cdist(v_batch, v_batch, p=2) ** 2

            for s_idx, scale in enumerate(scale_list):
                window = torch.exp(-dist2 / (2 * (scale**2)))
                weights = window.sum(dim=1, keepdim=True).clamp_min(1e-12)
                smoothed = window @ n_batch
                smoothed = smoothed / weights
                normals_s[mask, s_idx, :] = F.normalize(smoothed, p=2, dim=-1)

        uv_s = tangent_vectors(normals_s)  # (N, S, 2, 3)
        features = []

        for s_idx, scale in enumerate(scale_list):
            mean_curvature = torch.zeros(N, device=device, dtype=dtype)
            gauss_curvature = torch.zeros(N, device=device, dtype=dtype)
            ...
            for b in unique_batches:
                ...
                x_b = vertices[mask]
                n_b = normals_s[mask, s_idx, :]
                uv_b = uv_s[mask, s_idx, :, :]

                delta = x_b[None, :, :] - x_b[:, None, :]
                dist2 = (delta**2).sum(dim=-1)
                cos = (n_b @ n_b.T).clamp(-1, 1)
                d2_ij = dist2 * (2.0 - cos) ** 2
                window = torch.exp(-d2_ij / (2 * (scale**2)))

                P = torch.einsum("ikd,ijd->ijk", uv_b, delta)  # (Nb, Nb, 2)
                normals_delta = n_b[None, :, :] - n_b[:, None, :]
                Q = torch.einsum("ikd,ijd->ijk", uv_b, normals_delta)  # (Nb, Nb, 2)

                PPt = (
                    window[..., None, None] * (P.unsqueeze(-1) * P.unsqueeze(-2))
                ).sum(dim=1)
                PQt = (
                    window[..., None, None] * (P.unsqueeze(-1) * Q.unsqueeze(-2))
                ).sum(dim=1)
                ...
                S = torch.linalg.solve(PPt, PQt)
                a, b_, c, d = S[:, 0, 0], S[:, 0, 1], S[:, 1, 0], S[:, 1, 1]

                mean_curvature[mask] = (a + d).clamp(-1, 1)
                gauss_curvature[mask] = (a * d - b_ * c).clamp(-1, 1)

            features += [mean_curvature, gauss_curvature]
...
class dMaSIFConv(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, radius=1.0, hidden_units=None):
    ...
    def forward(self, points, nuv, features, ranges=None, k_neighbors=8):
        ...
        Args:
            ...
            k_neighbors (int, optional): number of nearest neighbors for sparse computation. Defaults to 32.
        ...
        # Torch-only path without KeOps.
        # Optimization: Pre-extract conv weights for F.linear (avoids Sequential overhead)
        conv_weight1, conv_bias1 = self.conv[0].weight, self.conv[0].bias
        conv_weight2, conv_bias2 = self.conv[2].weight, self.conv[2].bias

        features = self.net_in(features)
        # Optimization: Reduced transpose overhead - single unsqueeze/squeeze pattern
        features = (
            self.norm_in(features.unsqueeze(0).transpose(1, 2))
            .transpose(1, 2)
            .squeeze(0)
            .contiguous()
        )

        points_scaled = points / (sqrt(2.0) * self.Radius)
        normals = nuv[:, 0, :].contiguous()
        basis = nuv

        aggregated = torch.zeros(
            n_points, self.Hidden, device=points.device, dtype=features.dtype
        )

        # Try to use torch_scatter for faster aggregation if available
        try:
            from torch_scatter import segment_csr

            use_segment_csr = True
        except ImportError:
            use_segment_csr = False

        if ranges is not None:
            x_ranges = ranges[0] if isinstance(ranges, tuple) else ranges
            for b in range(x_ranges.shape[0]):
                start_idx = x_ranges[b, 0].item()
                end_idx = x_ranges[b, 1].item()
                n_batch_size = end_idx - start_idx

                if n_batch_size == 0:
                    continue

                p_batch = points_scaled[start_idx:end_idx]
                n_batch_t = normals[start_idx:end_idx]
                b_batch = basis[start_idx:end_idx]
                f_batch = features[start_idx:end_idx]

                k = min(k_neighbors, n_batch_size)
                dists = torch.cdist(p_batch, p_batch)
                _, neighbors = torch.topk(dists, k=k, dim=1, largest=False)

                src_local = (
                    torch.arange(n_batch_size, device=points.device)
                    .unsqueeze(1)
                    .expand(-1, k)
                    .reshape(-1)
                )
                dst_local = neighbors.reshape(-1)

                # Optimization: Use index_select instead of fancy indexing
                p_src = torch.index_select(p_batch, 0, src_local)
                p_dst = torch.index_select(p_batch, 0, dst_local)
                delta = p_dst - p_src

                n_src = torch.index_select(n_batch_t, 0, src_local)
                n_dst = torch.index_select(n_batch_t, 0, dst_local)

                # Optimization: Fused in-place operations for d2 and window
                dot = (n_src * n_dst).sum(dim=-1, keepdim=True)
                d2 = (delta * delta).sum(dim=-1, keepdim=True)
                d2.mul_((2.0 - dot).pow_(2))  # in-place mul
                window = d2.neg_().exp_()  # in-place neg and exp

                b_src = torch.index_select(b_batch, 0, src_local)
                local = torch.einsum("eij,ej->ei", b_src, delta)

                # Optimization: Use F.linear directly instead of Sequential
                local = F.linear(local, conv_weight1, conv_bias1).relu_()
                local = F.linear(local, conv_weight2, conv_bias2).relu_()

                f_dst = torch.index_select(f_batch, 0, dst_local)
                weighted = window * local * f_dst

                if use_segment_csr and n_batch_size > 0:
                    # Sort by source for segment_csr
                    sorted_src, perm = src_local.sort()
                    weighted_sorted = torch.index_select(weighted, 0, perm)
                    # Build CSR pointers
                    counts = torch.bincount(sorted_src, minlength=n_batch_size)
                    indptr = torch.zeros(
                        n_batch_size + 1, device=points.device, dtype=torch.long
                    )
                    indptr[1:] = counts.cumsum(0)
                    aggregated[start_idx:end_idx] = segment_csr(
                        weighted_sorted, indptr, reduce="sum"
                    )
                else:
                    aggregated[start_idx:end_idx].scatter_add_(
                        0, src_local.unsqueeze(-1).expand(-1, self.Hidden), weighted
                    )
        else:
            k = min(k_neighbors, n_points)
            dists = torch.cdist(points_scaled, points_scaled)
            _, neighbors = torch.topk(dists, k=k, dim=1, largest=False)

            src_idx = (
                torch.arange(n_points, device=points.device)
                .unsqueeze(1)
                .expand(-1, k)
                .reshape(-1)
            )
            dst_idx = neighbors.reshape(-1)

            # Optimization: Use index_select instead of fancy indexing
            p_src = torch.index_select(points_scaled, 0, src_idx)
            p_dst = torch.index_select(points_scaled, 0, dst_idx)
            delta = p_dst - p_src

            n_src = torch.index_select(normals, 0, src_idx)
            n_dst = torch.index_select(normals, 0, dst_idx)
            ...
            b_src = torch.index_select(basis, 0, src_idx)
            local = torch.einsum("eij,ej->ei", b_src, delta)
            ...
            f_dst = torch.index_select(features, 0, dst_idx)
            weighted = window * local * f_dst

            if use_segment_csr and n_points > 0:
                # Sort by source for segment_csr
                sorted_src, perm = src_idx.sort()
                weighted_sorted = torch.index_select(weighted, 0, perm)
                # Build CSR pointers
                counts = torch.bincount(sorted_src, minlength=n_points)
                indptr = torch.zeros(
                    n_points + 1, device=points.device, dtype=torch.long
                )
                indptr[1:] = counts.cumsum(0)
                aggregated = segment_csr(weighted_sorted, indptr, reduce="sum")
            else:
                aggregated.scatter_add_(
                    0, src_idx.unsqueeze(-1).expand(-1, self.Hidden), weighted
                )

        features = self.net_out(aggregated)
        # Optimization: Reduced transpose overhead - single unsqueeze/squeeze pattern
        features = (
            self.norm_out(features.unsqueeze(0).transpose(1, 2))
            .transpose(1, 2)
            .squeeze(0)
            .contiguous()
        )
