    ...
    if not bn:
        ...
        if use_in:
            ...
            return (
                ...
                LayerNormWrapper(64),
                LayerNormWrapper(64),
                LayerNormWrapper(emb_dims, two_d=False),
                LayerNormWrapper(512, two_d=False),
                LayerNormWrapper(256, two_d=False),
            )
        else:
            return (
                nn.BatchNorm2d(64),
...
class DGCNN(nn.Module):
    def __init__(
        ...
        (
            ...
            self.bn7,
            self.bn8,
        ) = get_batch_norm_layers(emb_dims, bn, use_in, use_gn, use_ln)

        self.conv1 = nn.Sequential(
            nn.Conv2d(dim_in * 2, 64, kernel_size=1, bias=False),
            self.bn1,
    ...
    def forward(self, surface):
        ...
        x, true_idx = get_graph_feature(
            x, k=self.k
        )  # (batch_size, dim_in, num_points) -> (batch_size, dim_in*2, num_points, k)
        x = self.conv1(
            x
        )  # (batch_size, dim_in*2, num_points, k) -> (batch_size, 64, num_points, k)
        x = self.conv2(
            x
        ...
        x, _ = get_graph_feature(
            x1, k=self.k, idx_unchanged=(true_idx if self.fixed_graph else None)
        ...
        x, _ = get_graph_feature(
            x2, k=self.k, idx_unchanged=(true_idx if self.fixed_graph else None)
        )  # (batch_size, 64, num_points)
        #                                                                                                  # -> (batch_size, 64*2, num_points, k)
        x = self.conv5(
            x
        ...
        x = torch.cat(
            (x, x1, x2, x3), dim=1
        )  # (batch_size, emb_dims + 64*3, num_points)

        x = self.conv7(
            x
        )  # (batch_size, emb_dims + 64*3, num_points) -> (batch_size, 512, num_points)
        x = self.conv8(
            x
        ...
        x = self.conv9(
            x
        )  # (batch_size, 256, num_points) -> (batch_size, dim_out, num_points)
...
class DGCNNLayer(nn.Module):
    def __init__(
        self, dim_in, dim_out, k=20, fixed_graph=False, emb_dims=1024, p_dropout=0.5
    ):
        super().__init__()
        super().__init__()
        self.k = k
