    ...
    def __init__(
        ...
        if level == "aminoacid":
            self.lin_feature1 = TwoLinear(
                num_radial * num_spherical, mid_emb, hidden_channels
            )
...
class ProNet(nn.Module):
    ...
    def forward(self, graph_data):
        ...
        batch_data = Batch.from_data_list(graph_data.misc_features["pronet_features"])

        # z, pos, batch = torch.squeeze(graph_data.x.long()), batch_data.coords_ca, graph_data.batch
        z, pos, batch = graph_data.x, batch_data.coords_ca, graph_data.batch
        ...
        b = (torch.cross(plane1, plane2) * (pos[refi0] - pos[i])).sum(dim=-1) / (
            (pos[refi0] - pos[i]).norm(dim=-1) + 1e-5
        )
        phi = torch.atan2(b, a)
