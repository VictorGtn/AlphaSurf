...
def _rbf(D, D_min=0.0, D_max=8.0, D_count=16):
    """
...
class BPGraphBatch:
    def __init__(self, surfaces, graphs, neighbors, num_gdf=16, mode="sg", neigh_th=8):
        ...
        neighbors = (
            torch.stack((neigh_verts, neigh_graphs))
            if mode == "sg"
...
def compute_bipartite_graphs(
    surfaces, graphs, neigh_th=8, k=16, use_knn=False, num_gdf=16
):
    """
    ...
    with torch.no_grad():
        ...
        for x in all_dists:
            ...
            neighbors_v.append(
                torch.stack((repeated_tensor_vert, min_indices_v.flatten()))
            )
            neighbors_g.append(
                torch.stack((repeated_tensor_graph, min_indices_g.flatten()))
            )
        ...
        bipartite_surfgraph = BPGraphBatch(
            surfaces, graphs, neighbors_v, mode="sg", num_gdf=num_gdf, neigh_th=neigh_th
        )
