...
class SurfaceGraphCommunication(nn.Module):
    ...
    def forward(self, surface=None, graph=None):
        ...
        has_surface_x = hasattr(surface, "x") and surface.x is not None
        has_graph_x = hasattr(graph, "x") and graph.x is not None
        if not has_surface_x and not has_graph_x:
            return surface, graph
        if not has_surface_x:
            if self.g_pre_block is not None:
                graph.x = self.g_pre_block(graph.x)
            return surface, graph
        if not has_graph_x:
            return surface, graph
        # get input features and apply preprocessing
        if self.s_pre_block is not None:
            surface.x = self.s_pre_block(surface.x)
        if self.g_pre_block is not None:
            graph.x = self.g_pre_block(graph.x)

        # == apply the message passing ==
        if self.bp_sg_block is not None and self.bp_gs_block is not None:
            # prepare the communication graph (with caching)
            bp_gs_batch_container, bp_sg_batch_container = self.compute_graph(
                surface, graph
            )
        ...
        if self.s_post_block is not None:
            xs = self.s_post_block(surface.x, xs_out)
        else:
            xs = surface.x
        if self.g_post_block is not None:
