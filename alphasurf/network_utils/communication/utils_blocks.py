...
from alphasurf.network_utils.misc_arch.gvp_gnn import GVPConv
...
def init_block(
    ...
    if name == "linear":
        ...
        return NoParamAggregate(
            aggr=aggr, add_self_loops=add_self_loops, fill_value=fill_value
        )
...
class HMRWrapper(nn.Module):
    ...
    def forward(self, x, bpgraph):
        ...
        expanded_message_features = torch.cat(
            [all_feats, bpgraph.encoded_dists, bpgraph.encoded_angles], axis=-1
        )
        ...
        h_chem_geom = scatter(
            h_chem_geom, neigh_out, dim=0, reduce="sum", dim_size=len(x)
        )
        return h_chem_geom
...
class GVPWrapper(nn.Module):
    def __init__(
        ...
        out_dims = dim_out, 3
        self.gvp = GVPConv(
            in_dims, out_dims, edge_dims, n_layers=n_layers, vector_gate=vector_gate
        )
    ...
    def forward(self, x, bpgraph):
        ...
        x_o_s, x_o_v = self.gvp(
            x=x, edge_index=bpgraph.edge_index, edge_attr=(e_s, e_v)
        )
        return x_o_s
...
class HMR2LayerMLPChunk(nn.Module):
    def __init__(self, dim_in, hdim, dropout):
        super().__init__()
        self.net = HMR2LayerMLP(
            dim_in=dim_in, dim_mid=hdim, dim_out=2 * hdim, dropout=dropout
        )
        self.sigmoid = nn.Sigmoid()
        self.softplus = nn.Softplus()
