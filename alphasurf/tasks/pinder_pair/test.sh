...
# 3. Raise FD limit for multiprocessing dataloader
ulimit -n 65536

# 4. Temp & cache to SCRATCH
...
# 7. On-the-fly args (no noise, no igl normals)
ON_FLY_ARGS="on_fly.surface_method=${SURFACE_METHOD:-alpha_complex} \
             ...
             +on_fly.surface_label_mode=${SURFACE_LABEL_MODE:-dist} \
...
python -u -X faulthandler $REPO_ROOT/tasks/pinder_pair/test.py \
    ...
    run_name=${RUN_NAME:-pinder_test} \
    "+ckpt_path='${CKPT_PATH}'" \
    "+common_only=${COMMON_ONLY:-false}" \
    "+dump_per_system=${DUMP_PER_SYSTEM:-null}"
