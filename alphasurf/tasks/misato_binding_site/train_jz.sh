...
srun --export=ALL "$CONDA_PREFIX/bin/python" -m alphasurf.tasks.misato_binding_site.train \
    ...
    eval_frame_mode="${EVAL_FRAME_MODE:-first}" \
    train_frame_index="${TRAIN_FRAME_INDEX:-0}" \
    eval_frame_index="${EVAL_FRAME_INDEX:-0}" \
    train_frame_fraction="${TRAIN_FRAME_FRACTION:-0.5}" \
    eval_frame_fraction="${EVAL_FRAME_FRACTION:-0.5}" \
