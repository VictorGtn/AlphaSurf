#!/bin/bash
#SBATCH --job-name=misato_guo_batch64
#SBATCH -A pyg@h100
#SBATCH -C h100
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
output_path=$REPO_ROOT/tasks/misato_binding_site/log/guo_batch64_${SLURM_JOB_ID}.json

cd "$REPO_ROOT/.."
srun --export=ALL "$CONDA_PREFIX/bin/python" \
    -m alphasurf.tasks.misato_binding_site.evaluate_guo_batch64 \
    data_dir="$DATA_DIR" \
    md_path="$DATA_DIR/MD.hdf5" \
    split_dir="$DATA_DIR/splits" \
    hydra.searchpath="[file://$REPO_ROOT/tasks/shared_conf]" \
    cfg_surface.tufting=true \
    loader.batch_size="${BATCH_SIZE:-128}" \
    loader.num_workers="${NUM_WORKERS:-20}" \
    loader.persistent_workers=false \
    loader.prefetch_factor=2 \
    "+guo_checkpoint_paths=[$checkpoint_list]" \
    "+guo_output=$output_path"
