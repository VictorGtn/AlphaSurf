...
def get_default_input(in_dim_surface, in_dim_graph, model_dim=128, dropout=0.1):
    ...
    s_post_block = CatMergeBlock(
        net=HMR2LayerMLP(model_dim * 2, model_dim, model_dim, dropout)
    )
    g_post_block = CatMergeBlock(
        net=HMR2LayerMLP(model_dim * 2, model_dim, model_dim, dropout)
    )
    input_block = SurfaceGraphCommunication(
...
def get_middle_block(model_dim=128, dropout=0.1):
    ...
    mp_block = SurfaceGraphCommunication(
        s_pre_block=s_pre_block,
        g_pre_block=g_pre_block,
        bp_sg_block=sg,
        bp_gs_block=gs,
...
def get_default_model(
    ...
    block_list = [
        get_default_input(
            in_dim_surface, in_dim_graph, model_dim=model_dim, dropout=dropout
        )
    ]
