...
def get_lr_scheduler(scheduler, optimizer, **kwargs):
    ...
    if scheduler == "PolynomialLR":
        decay_scheduler = PolynomialLR(
            optimizer, total_iters=total_epochs - warmup_epochs, power=1
        )
        ...
        decay_scheduler = ReduceLROnPlateau(
            optimizer, patience=kwargs["patience"], factor=kwargs["factor"], mode="max"
        )
    else:
...
class AtomPLModule(pl.LightningModule):
    ...
    def step(self, batch):
        raise NotImplementedError(
            "Each subclass of AtomPLModule must implement the `step` method"
        )
    ...
    def configure_optimizers(self):
        ...
        scheduler_obj = get_lr_scheduler(
            scheduler=sched_params.name,
            optimizer=optimizer,
